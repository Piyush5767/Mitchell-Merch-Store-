function isOverflowed(obj){
		var Pheight = '',Pwidth = '',ItemLeft = '',ItemTop = '',ItemWidth = '',ItemHeight = '';
		Pheight 	= obj.parHeight;
		Pwidth  	= obj.parWidth;
		ItemLeft 	= obj.dragLeft;
		ItemHeight  = obj.dragHeight;
		ItemWidth   = obj.dragWidth;
		ItemTop     = obj.dragTop;
		ItemAreaWidth = ItemLeft+ItemWidth;
		ItemAreaHeight = ItemHeight+ItemTop;
		if((ItemLeft < 0) || (ItemTop < 0) || ( ItemAreaWidth > Pwidth) || (ItemAreaHeight > Pheight)){
			jQuery('#alrtmsg').addClass('active').html('<p class="text-danger"><strong>'+alrtmsg+'</strong></p>').fadeIn().delay(5000).fadeOut();
		}		
	setTimeout(function(){
		jQuery('#alrtmsg').removeClass('active');	
	},5000);
}
...
var design={	
	...
	ini:function(){
		...
		$jd('.add_item_design').click(function(){
			self.designer.art.Designcategories(true, 0);
			if( jQuery('#dag-list-designarts').html() == '')
				self.designer.art.designarts('');
		});
		...
		$jd('.upload_images').click(function(){
			self.designer.art.uploadedDesigns();			
		});
		
		$jd('#dag-art-designdetail button').click(function(){
			jQuery('#dag-art-designdetail').hide('slow');
			jQuery('#dag-list-designarts').show('slow');
			jQuery('#arts-designadd').hide();
			jQuery('#designarts-pagination').css('display', 'block');
		});
	...
	ajax:{
		form: function(){
			...
			datas.Customcliparts = design.exports.Customcliparts();
			datas.texts = design.exports.texts();
			datas.images = design.exports.designimage(); 
			datas.size   = jQuery('#productSize').val();			
			return datas;
		},
		getPrice: function(){
			...
			var datas = this.form();
			if(designid != ''){				
				datas.designId = designid;
				datas.request = tempvar;
				jQuery.ajax({
					type: "POST",
					dataType: "json",
					url: baseURL + "cart/designPrice", 
					data:datas,					
				}).done(function( data ) {
					if (data != '')
					{
						if (typeof data.sale != 'undefined')
						{
							...
							if(tempvar == 0){
								jQuery('#productSize').val(data.selected);
								jQuery('#quantity').val(data.quantity);		
								var iclr = 0;
								jQuery('.list-colors span').each(function(){
									jQuery(this).removeClass('active');
									var clr = jQuery(this).data('color');
									if(clr == data.color){
										if(colorChk == 0){
											jQuery(this).addClass('active');
											design.products.changeColor(this, iclr);
											colorChk = colorChk+1;
										}										
									}
									iclr = iclr+1;
		...
		addToCart: function(){
			var options		= {};
			options.size = jQuery('#productSize').val();
			options.vectors	= JSON.stringify(design.exports.vector());
			options.prdcolor = jQuery('#product-list-colors span.active').data('color');
		
			options.images	= {};
			if ((typeof design.output.front != 'undefined') || (design.output.front_canvsImgexists == true))
				options.images.front = design.output.front.toDataURL();
				options.images.canvfront = design.output.front_canvs;
				options.images.canvfrontExists = design.output.front_canvsImgexists;
				
			if ((typeof design.output.back != 'undefined') || (design.output.back_canvsImgexists == true))
				options.images.back = design.output.back.toDataURL();
				options.images.canvback = design.output.back_canvs;
				options.images.canvbackExists = design.output.back_canvsImgexists;
				
			if (typeof design.output.left != 'undefined')
				options.images.left = design.output.left.toDataURL();
				options.images.canvleft = design.output.left_canvs;
				options.images.canvleftExists = design.output.left_canvsImgexists;
				
			if (typeof design.output.right != 'undefined')
				options.images.right = design.output.right.toDataURL();
				options.images.canvright = design.output.right_canvs;
				options.images.canvrightExists = design.output.right_canvsImgexists;
			
			var datas = design.ajax.form();
			datas.design = options;				
			datas.teams = design.teams;				
			datas.fonts = design.fonts;	
			datas.design_id = designid;
			jQuery.ajax({
				type: "POST",	
				processData: false,	
	...
	designer:{
		art:{
			...
			Designcategories: function(load, index){
				if (typeof index == 'undefined') index = 0;
				self = this;
				...
				if (typeof load != 'undefined' && load == true)
				{
					jQuery('#dag-art-designcategories').children('ul').each(function(){
						if (index == jQuery(this).data('type'))
						{
							ajax = false;
						}
				...
				if (ajax == true)
				{					
					jQuery('#dag-art-designcategories').addClass('loading');
					jQuery.ajax({				
						dataType: "json",
						url: baseURL + "designart/categories/"+index
					}).done(function( data ) {						
						if (data != '')
						{								
							var e = document.getElementById('dag-art-designcategories');
							var html = self.treeCategories(data, e, index);							
						}
					}).always(function(){
						jQuery('#dag-art-designcategories').removeClass('loading');
					});					
				}
			},	
			uploadedDesigns: function()
			{
				var parent = document.getElementById('uploaded-arts');				
				if(user_id != 0){
					parent.innerHTML = '';
					var page = jQuery('#uploadart-number-page').val();				
					jQuery.ajax({
						type: "POST",
						data: { page: page},
						dataType: "json",					
						url: baseURL + "designart/designupload",
					}).done(function(data){
						if (data.imageobj.length > 0)
						{
							var button = document.createElement('button');
							button.setAttribute('onclick','design.designer.art.rmvimgs()');
							button.setAttribute('title','Remove Images');
							button.setAttribute('data-original-title','Remove');
							button.innerHTML = '<i class="glyphicon glyphicon-trash"></i>';							
							button.id = 'rmvimgs';
							button.className = 'btn btn-danger tooltips';	
							parent.appendChild(button);
							
							var h4 = div = document.createElement('h4');
							h4.className = 'text-center';
							h4.innerHTML = 'Uploaded Images';
							parent.appendChild(h4);							

							jQuery.each(data.imageobj, function(i, art){							
								var div = document.createElement('div');
									div.className = 'col-xs-3 col-md-3 box-art';									
								var span = document.createElement('span');
								span.className = 'dltimg';
								span.innerHTML = '<input class="imgid" type="checkbox" value="'+art.id+'">';
								div.appendChild(span);
								var a = document.createElement('a');
									a.setAttribute('class', 'thumbnail');
									a.setAttribute('href', 'javascript:void(0)');
									a.setAttribute('onclick', 'design.myart.create(this)');
									art.imgThumb = baseURL + art.imgthumb;
									art.imgMedium = baseURL + art.imgfull;
									art.title = '';
									art.type  = 'uploadedimage';
									art.url   = baseURL+art.imgfull;
									art.clipart_id    = art.id;
									art.file_name = '';
									art.thumb = baseURL+art.imgthumb;								
									a.item = art;
								var img = '<img alt="" src="'+baseURL + art.imgthumb+'">';
								a.innerHTML = img;
								div.appendChild(a);
							...
							if (data.counts > 1)
							{
								jQuery('#prev-uploaded-pagi').css('display', 'block');
								var ul = jQuery('#prev-uploaded-pagi .pagination');
								ul.html('');
								for(var i=1; i<= data.counts; i++)
								{
									...
									jQuery(li).click(function(){
										...
										{
											jQuery('#uploadart-number-page').val( jQuery(this).data('id') );
											self.uploadarts();
										}
									});
								}
					...
					parent.innerHTML = '<p><strong> Please login to get all your previous uploaded images </strong></p>';
				}
			},
			rmvimgs: function(){
				var ids = [];
				jQuery('.imgid').each(function(){
				...
				jQuery.ajax({
					...
					data: {ids: ids},
					dataType: "json",					
					url: baseURL + "designart/remove",
				}).done(function(data) {					
					if(data == true){
						Redirect(baseURL+'design');
					}
				});
			},			
			...
			designarts: function(cate_id)
			{
				var self = this;
				var parent = document.getElementById('dag-list-designarts');
				parent.innerHTML = '';
				jQuery('#dag-art-designdetail').hide('slow');
				jQuery('#dag-list-designarts').show('slow');
				jQuery('#arts-designadd').hide();
				jQuery('#dag-list-designarts').addClass('loading');
				...
				jQuery.ajax({
					...
					data: { page: page, keyword: keyword},
					dataType: "json",					
					url: baseURL + "designart/arts/"+cate_id
				}).done(function( data ) {					
					if (data == null)
					{
						jQuery('#dag-list-designarts').removeClass('loading');
						parent.innerHTML = 'Data not found!';
						var ul = jQuery('#designarts-pagination .pagination').html('');
						jQuery('#designart-number-page').val(0);
						return false;
					}
					if (data.arts.length > 0)
					{
						jQuery.each(data.arts, function(i, art){
							...
							var a = document.createElement('a');
								...
								a.setAttribute('class', 'thumbnail');
								a.setAttribute('href', 'javascript:void(0)');
								a.setAttribute('onclick', 'design.designer.art.DesignartDetail(this)');
								jQuery(a).data('id', art.clipart_id);
								jQuery(a).data('type', 'cstmdsgn');
								jQuery(a).data('clipart_id', art.clipart_id);
								jQuery(a).data('price', art.price);
								jQuery(a).data('medium', url + art.url);
								art.imgThumb = url +art.thumb;
								art.imgMedium = url + art.url;
								a.item = art;
							var img = '<img alt="" src="'+art.imgThumb+'">';
							a.innerHTML = img;
							div.appendChild(a);
							parent.appendChild(div);
						});						  
						...
						{
							jQuery('#designarts-pagination').css('display', 'block');
							var ul = jQuery('#designarts-pagination .pagination');
							ul.html('');
							for(var i=1; i<= data.count; i++)
							{
								var li = document.createElement('li');
								...
								jQuery(li).click(function(){
									if ( jQuery(this).hasClass('active') == false )
									{
										jQuery('#designart-number-page').val( jQuery(this).data('id') );
										self.arts(cate_id);
									}
								});
							}
			...
			DesignartDetail: function(e)
			{
				var id = jQuery(e).data('id');
				jQuery('.box-art-detail').css('display', 'none');
				jQuery('#designarts-pagination').css('display', 'none');
				if (document.getElementById('art-detail-'+id) == null)
				{
					...
					jQuery.ajax({
						...
						if (typeof data.error != 'undefined' && data.error == 0)
						{
							...
							jQuery('.art-detail-price').html('Price ' + data.price.currency_symbol + data.price.amount);							
						}					
				...
				jQuery('#dag-list-designarts').hide('slow');
				jQuery('#dag-art-designdetail').show('slow');
				jQuery('#arts-designadd').show();
				jQuery('#arts-designadd button').unbind('click');
				jQuery('#arts-designadd button').bind('click', function(event){design.art.create(e);});
				jQuery('#arts-designadd button').button('reset');
			},			
				...
				jQuery.each(categories, function(i, cate){
					var li = document.createElement('li'),
						...
						jQuery(a).click(function(){
							jQuery('#dag-art-categories a,#dag-art-designcategories a').removeClass('active');
							jQuery(a).addClass('active');
							jQuery('#designart-number-page').val(0);
							jQuery('#designarts-pagination .pagination').html('');
							self.arts(cate.id);
						});
	...
	products:{
		...
		changeDesign: function(e){
			...
			jQuery.ajax({
				...
				if (data.status == 1)
				{
					...
					jQuery('#product-sizeattributes').html(data.product.sizeAttribute);	
					jQuery('#productFiletype').val(data.product.printful_printfile);
					design.item.designini(items);
		...
		changeProduct: function(e, product){
			var id 	= jQuery(e).data('id');
			jQuery('.product-list,.product-pagi').css('display','none');
		...
		productCate: function(id){
			...
			if (typeof seft.products[id] != 'undefined'){
				...
				jQuery('.dataprogesss').addClass('loading');
				jQuery('.product-list').css('display','none');
				jQuery.ajax({
					type: "POST",
					dataType: "json",
					...
					jQuery('.dataprogesss').removeClass('loading');
		...
		addProduct: function(products){			
			...
			if (products.length == 0){
				jQuery('.product-pagi').css('display','none'); 				
			}else{	
				var ij = 1;
				var seft = this;
				jQuery.each(products, function(i, product){  
					...
					html = '<div class="id" style="display:none;">'+ij+'</div><div class="thumbnail"><img src="'+baseURL+product.image+'" alt="'+product.title+'" class="img-responsive"> <div class="caption">' + product.title +'</div></div>';
						div.innerHTML = html;					
				...
				if(products.length > 5)	
					jQuery('.product-pagi').css('display','block');
				else
					jQuery('.product-pagi').css('display','none');
				
				var options = {
				  valueNames: ['id','thumbnail'],
				  item: '<li><div class="id" style="display:none;"></div><div class="thumbnail"></div></div></li>',
				  page: 8,
				  plugins: [
					ListPagination({}),
				  ],
	...
	art:{
		create: function(e){
			jQuery('#arts-add button,#arts-designadd button').button('loading');
			var item = e.item;
			$jd('.ui-lock').attr('checked', false);
			var img = $jd(e).children('img');			
			var o 			= {};
			o.type 			= 'clipart';
			o.clipartTpe    = jQuery(e).data('type');
			o.upload 		= 0;			
			...
			if (item.file_type != 'svg')
			{
				...
				img.onload = function(){				  
					...
					var content = '<svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" xmlns:xlink="http://www.w3.org/1999/xlink">'
									 ...
									 + '</svg>';
					o.svg 		= jQuery.parseHTML(content);	
					jQuery('#arts-add button,#arts-designadd button').button('reset');
					design.item.create(o);
					$jd('.modal').modal('hide');
				}
				var src = item.imgMedium;
				src = src.replace('http://', '');
				img.src = urlCase +'?src='+ src +'&w=250&h=auto&q=90';					
			}
			else
			{
				$jd.ajax({
	...
	item:{
		...
		create: function(item){		
			...
			span.style.right    = center.left+'px';
			span.style.bottom   = center.top+'px';
		...
		move: function(e){
			...
			e.draggable({/*containment: "#dg-designer", */scroll: false, 
				drag:function(event, ui){
					...
					obj.parHeight  = height;
					obj.parWidth   = width;
					obj.dragLeft   = $left;
					obj.dragTop    = $top;
					obj.dragWidth  = $width;
					obj.dragHeight = $height;
					if(jQuery('#alrtmsg').hasClass('active') == false){
						isOverflowed(obj);
					}
				},
	...
	svg:{		
		...
		items: function(postion, callback)
		{
			...
			design.svg.itemsCanvas(postion);
			return canvas;
		},
		itemsCanvas: function(postion)
		{
			...
			var DesignHeight = '' ,DesignWidth = '',DesignLeft = '', DesignTop = productType = '';
			var inchWidth = '', inchHeight = '', parentWidth = '',parentHeight = '',dpi = '';		
			
			// Design Area Width & Height
			DesignHeight 	= design.convert.px(jQuery('#view-'+postion+' .design-area').css('height'));
			DesignWidth  	= design.convert.px(jQuery('#view-'+postion+' .design-area').css('width'));
			DesignLeft   	= design.convert.px(jQuery('#view-'+postion+' .design-area').css('left'));
			DesignTop  		= design.convert.px(jQuery('#view-'+postion+' .design-area').css('top'));
			
			// Convert inch to pixel			
			printfulsizediffer 		= jQuery('#productSize').data('printfulsizediffer');
			dpi						= jQuery('#productSize option:selected').data('dpi');			
			if((printfulsizediffer == 1) && (postion ==  'back')){
				inchWidth 			= jQuery('#productSize option:selected').data('back_width');
				inchHeight			= jQuery('#productSize option:selected').data('back_height');
				inchTotalwidth      = jQuery('#productSize option:selected').data('back_totalwidth');
				inchTotalheight     = jQuery('#productSize option:selected').data('back_totalheight');				
			}else{
				inchWidth 			= jQuery('#productSize option:selected').data('width');
				inchHeight			= jQuery('#productSize option:selected').data('height');
				inchTotalwidth      = jQuery('#productSize option:selected').data('totalwidth');
				inchTotalheight     = jQuery('#productSize option:selected').data('totalheight');				
			}			
			
			productfileType 	= jQuery('#productFiletype').val();			
			parentWidth  = inchWidth * dpi;
			parentHeight = inchHeight * dpi;
			
			//parentWidth = parentHeight = 2250;
			
			if(parentWidth == '' || isNaN(parentWidth) || parentWidth <= 0){ parentWidth = 1800;}
			if(inchHeight == ''  || isNaN(inchHeight)  || inchHeight <= 0){  parentHeight = 1800; }
			
			//console.log('Width: '+parentWidth+' Height: '+parentHeight);	
			jQuery('#view-' +postion+ ' .design-area .drag-item').each(function(){				
				dragItem = true;
				var Canvasleft = '' ,Canvastop = '',Canvaswidth = '', Canvasheight = '';
				var perTop = '', perLeft = '', perWidth = '',perHeight = '';
				var perTop = '', pixelsLeft = '', pixelsWidth = '',pixelsHeight = '';
				typed = jQuery(this);				
				obj[i] 			= {};
				obj[i].typeData = typed[0].item.type;
				
				Canvastop 		= design.convert.px(jQuery(this).css('top'));
				Canvasleft 		= design.convert.px(jQuery(this).css('left'));
				Canvaswidth 	= design.convert.px(jQuery(this).css('width'));
				Canvasheight 	= design.convert.px(jQuery(this).css('height'));
 				
				//console.log('Svg Width: '+Canvaswidth+' Svg Height: '+Canvasheight);
				if(isNaN(Canvastop)){   Canvastop = 0;    }
				if(isNaN(Canvasleft)){ 	 Canvasleft = 0;  }
				...
				//console.log('Design Width: '+DesignWidth+' Design Height: '+DesignHeight);
				
				// Calculating % for large scale image
				perTop 			= (Canvastop / DesignHeight) * 100;
				perLeft 		= (Canvasleft / DesignWidth) * 100;
				perWidth 		= (Canvaswidth / DesignWidth) * 100;
				perHeight 		= (Canvasheight / DesignHeight) * 100;		

				if(perTop 	 > 100){ perTop = 100;}
				if(perLeft 	 > 100){ perLeft = 100;}
				if(perWidth  > 100){ perWidth = 100;}
				if(perHeight > 100){ perHeight = 100;}				
				//console.log('Svg Per Width: '+perWidth+' Svg Per Height: '+perHeight);
				
				// Calculating pixel for large image size
				pixelsTop		= parentHeight*(perTop/100);
				pixelsLeft 		= parentWidth*(perLeft/100);
				
				if(parentWidth > parentHeight){
					pixelsWidth 	= parentHeight *(perWidth/100);
				}else{
					pixelsWidth 	= parentWidth *(perWidth/100);
				}
				
				pixelsHeight 	= parentHeight*(perHeight/100);					
				//console.log('Svg Pixel Width: '+pixelsWidth+' Svg Pixel Height: '+pixelsHeight);
				
				obj[i].top 		= pixelsTop;
				obj[i].left 	= pixelsLeft;
				obj[i].width 	= pixelsWidth;
				obj[i].height 	= pixelsHeight;	
				...
				var svg 		= jQuery(this).find('svg');	
				obj[i].svg 		= jQuery('<div></div>').html(jQuery(svg).clone().attr({'width':pixelsWidth+'px','height':pixelsHeight+'px'})).html();
				var image 		= jQuery(svg).find('image');
				if (typeof image[0] == 'undefined')
				{
					obj[i].img 	= false;
			...
			if(productfileType=='single'){
				var dragItem = true;				
			}	
			design.output[postion+'_canvsImgexists'] = dragItem;			
			obj.sort(function(obj1, obj2) {	
				return obj1.zIndex - obj2.zIndex;
			});
			...
			function convertCanvasToImage(canvas,postion){
				if((inchTotalwidth != inchWidth) || (inchTotalheight != inchHeight)){
					canvWidth  = inchTotalwidth * dpi;
					canvHeight = inchTotalheight * dpi;	
					var canvs			= document.createElement('canvas');
						canvs.width 	= canvWidth;
						canvs.height 	= canvHeight;
					...
					var ctx = canvs.getContext('2d');
						...
						img.onload = function(){	 
							ctx.drawImage(img,canvs.width / 2  - img.width / 2,canvs.height / 2 - img.height / 2);	
							var image = new Image();
							image.src = canvs.toDataURL("image/png");					
							design.output[postion+'_canvs'] = image.src;						
						};				
				}else{
					var image = new Image();
			...
			convertCanvasToImage(canvas,postion);			  
		},		
	...
	saveDesign: function(){	
		...
		if(cartPrice != 'false'){
			...
			var requrl = baseURL+"user/saveDesign";
			jQuery.ajax({
				url: requrl,
				type: "POST",
	...
	save: function(){
		...
		{	
			if (user_id == design.designer_id && cartEdit == false)
			{
				...
				if (jQuery('#view-'+view +' .product-design').html().length > 10)
				{
					if (jQuery('#view-'+view+' .content-inner').html() != '')
					{
						jQuery('#view-'+view+' .drag-item').each(function(){
							if (typeof this.item.clipart_id != 'undefined' && this.item.clipartTpe != 'cstmdsgn')
		...
		designimage: function(){
			...
			jQuery.each(['front', 'back', 'left', 'right'], function(i, view){
				...
				{	
					var i = 0;
					jQuery('#view-'+ view).find('.drag-item').each(function(){						
						if (this.item.type == 'clipart' && this.item.clipartTpe != 'cstmdsgn'){
			...
			jQuery.each(postions, function(i, postion){
				...
				{					
					...
					jQuery('#view-'+ postion).find('.drag-item').each(function(){
						...
						item.clipartTpe = this.item.clipartTpe;
...
jQuery(document).ready(function(){
	...
	jQuery(document).on('change','#productSize,#quantity',function(){
		if (jQuery(this).is("#quantity")) {
			design.ajax.getPrice();	
		}else{
			var pricedep = jQuery(this).data('prdtpricedep');
			if(pricedep == 1){
				design.ajax.getPrice();		
			}
		}
