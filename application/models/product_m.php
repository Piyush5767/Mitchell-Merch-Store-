...
class Product_m extends MY_Model
{
	...
	*/
	...
	public function getSizeAttribute($product_id){		
		$this->_timestamps	= false;			
		$this->_order_by	= 'id DESC';	
		$this->_table_name 	= 'sizeattributes';
		$this->db->where('product_id', $product_id);	
		$this->db->limit(1);			
		$row = parent::get(null, true);		
		if(isset($row->id)){			
	...
	public function getNew($type = '')
	{
		switch( $type ){
			...
			default:
				...
				$data->sku 	= ''; 
				$data->printful_productid 	= '';
				$data->printfulClrCode 	 = 0;
				$data->printfulprcdepend = 1;
				$data->printfulsize_differ = 0;				
				$data->printful_printfile 	= 'multiple';				
				$data->created 		= '';  
				$data->print_type	= '';
				$data->price 		= '';
				$data->sale_price	= '';
