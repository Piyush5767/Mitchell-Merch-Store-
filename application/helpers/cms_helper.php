...
function pre($arr = array(),$die = false){
...
// get url of image in custom clipart
function imageCustomArt($art)
{	
	$url  = site_url('media/designimage');
	$images = new stdClass();
	$images->thumb  	= $url.'/thumb/'.$art->thumb;
	$images->medium 	= $url.'/'.$art->full;
	...
	if(count($categories))
	{
		...
		foreach($categories as $category)
		{
			...
			if($show == 'yes')
			{
				if($level == 0)
				{
					$html .= '<li><a href="'.site_url().$link.$category->id.'-'.$category->slug.'" style="display: block;"><div class="text-center clear"><img '.$style.' class="img-thumbnail" src="'.$category->image.'" alt="'.$category->title.'"/></div><div class="text-center clear"><h4>'.$category->title.'</h4></div></a></li>';
				}
...
function designimage($data, $prefix = '')
{
	$path = ROOTPATH .DS. 'media' .DS. 'designimage' .DS;
	$CI = get_instance();
	$CI->load->library('file');
	$file 		= new file();
