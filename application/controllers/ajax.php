...
class Ajax extends Frontend_Controller {
	...
	public function design($key = '')
	{
		...
		{
			if(strpos($key,'cart-id-') !== false){
				$exp = explode('cart-id-',$key);
				...
				if(!empty($cartObj) && array_key_exists($exp[1],$cartObj)){
					...
					$design->id = null;
					$design->image = '';
					$design->user_id = $cart['user_id'];
					$design->design_id = 0;
					$design->fonts = $cart['fonts'];
					$design->vectors = $cart['vector'];
					$design->teams  = '';	
	...
	{		
		...
		else
		{
			...
			$image->change_color= 0;
			$imgfull            = 'media/assets/uploaded/'. $year .'/'. $month .'/'. $image->file_name;
			$image->url			= site_url() .$imgfull;
			...
			$this->image_lib->resize();
			$thumb 				= str_replace($this->image_lib->dest_folder, '', $this->image_lib->full_dst_path);
			$imgthumb           = 'media/assets/uploaded/'. $year .'/'. $month .'/'. $thumb;
			$image->thumb 		= site_url().$imgthumb; 
			...
			$this->users_m->saveuser_images(array('usrid'=>$usr_id,'imgfull'=>$imgfull,'imgthumb'=>$imgthumb));			
		}			
		echo json_encode(array('status' => $status, 'msg' => $msg));
	}
}
