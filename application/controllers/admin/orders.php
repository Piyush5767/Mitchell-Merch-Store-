...
class Orders extends Admin_Controller
{
	...
	{
		$this->data['breadcrumb'] = lang('orders_admin_refund_transaction_tab_title');
        $this->data['meta_title'] = lang('orders_admin_refund_transaction_tab_title');
        $this->data['sub_title'] = '';
		
		// load settting
		$this->load->model(array('settings_m','refund_m','payment_m'));
		$row 	= $this->settings_m->getSetting();
		$setting = json_decode($row->settings);
		...
		// pagination
		$this->load->library('pagination');
		$config['base_url'] 		= site_url('admin/orders/refundorders');
		$config['total_rows']		= $this->refund_m->getRefunds(true, 5, 1,$this->session->userdata('search_order'),$this->session->userdata('option_order'));
		...
		$orders = $this->refund_m->getRefunds(false, $config['per_page'], $this->uri->segment(4), $this->session->userdata('search_order'),$this->session->userdata('option_order'));
		$this->data['orders'] = $orders; 
		$payments = $this->payment_m->getData();	
		$payments_arr = array();
		if(!empty($payments)){
			foreach($payments as $pay_key=>$pay_row){
				$payments_arr[$pay_row->id] = $pay_row->title;				
			}
		}
		
		$this->data['payments'] = $payments_arr;
		
		// Load view
		$this->data['subview'] = 'admin/order/refundorders';
		$this->load->view('admin/_layout_main', $this->data);		
	}
	
	public function refundFailure($id)
	{
		if((int)$id == 0)
			redirect('admin/order/refundfailure');
			
		$this->data['breadcrumb'] = lang('orders_admin_refund_failure_tab_title');
		$this->data['meta_title'] = lang('orders_admin_refund_failure_tab_title');
		$this->data['sub_title']  = lang('orders_admin_transaction_status');
		
		$this->load->model(array('settings_m','refund_m','payment_m','order_m'));
		$row 	= $this->settings_m->getSetting();
		$setting = json_decode($row->settings);		
		$this->data['setting'] 	= $setting;	
		
		// get order detail
		$refundorder 	= $this->refund_m->getRefund($id);
		$order 			= $this->order_m->getOrderByItemNumber($refundorder->order_number,true);		
		$this->data['refundorders'] = $refundorder; 
		$this->data['order'] 		= $order; 
		
		if(count($refundorder) == 0)
		{		
		...
		// get payment method
		$payment	= $this->payment_m->get($refundorder->payment_id, true);
		$this->data['payment'] = $payment;		
				
		// Load view
		$this->data['subview'] = 'admin/order/refundfailure';
		$this->load->view('admin/_layout_main', $this->data);		
	}	
	...
	{		
		...
		$setting = json_decode($row->settings);
		$this->data['setting'] = $setting;
		$this->data['order_cliparts'] = $this->order_m->getCliparts_tr($id);				
		// get histories
		...
		$this->data['printfulResponse'] = $this->printful_m->getResponse($id);
	...
	function transacationDelete($id = '')
	{
		$this->load->model('refund_m');
		$this->refund_m->_table_name = "refund_transaction";
		if($id != '')
		{			
			$where = array(
				'id'=>$id,
			);
			if($this->refund_m->deleteTransaction($where))
			{	
				$this->session->set_flashdata('msg', lang('orders_admin_delete_transaction_success_msg'));
				redirect(site_url('admin/orders/refundorders'));	
			}
			...
			foreach($checkb as $id)
			{
				...
				$this->refund_m->deleteTransaction($where);		
			}
			$this->session->set_flashdata('msg', lang('orders_admin_delete_transaction_success_msg'));
			redirect(site_url('admin/orders/refundorders'));			
		}		
		$this->session->set_flashdata('error', lang('orders_admin_delete_transaction_error_msg'));
		redirect(site_url('admin/orders/refundorders'));		
	}
