...

{
...

	{
...

		{
			...
			$sizeattribute 				= $this->product_m->getSizeAttribute($id);	
		...
		if(isset($sizeattribute) && $sizeattribute !== false)

		{			

			$sizeattr 					= array();

			$sizeattr['dpi'] 			= json_decode($sizeattribute->dpi);

			$sizeattr['size'] 			= json_decode($sizeattribute->size);

			$sizeattr['height'] 		= json_decode($sizeattribute->height);
			
			$sizeattr['totalheight'] 	= json_decode($sizeattribute->totalheight);
			
			$sizeattr['width'] 			= json_decode($sizeattribute->width);

			$sizeattr['totalwidth'] 	= json_decode($sizeattribute->totalwidth);
			
			$sizeattr['back_height'] 		= json_decode($sizeattribute->back_height);
			
			$sizeattr['back_totalheight'] 	= json_decode($sizeattribute->back_totalheight);
			
			$sizeattr['back_width'] 		= json_decode($sizeattribute->back_width);

			$sizeattr['back_totalwidth'] 	= json_decode($sizeattribute->back_totalwidth);			
			
			$sizeattr['price'] 			= json_decode($sizeattribute->price);			
		}
...

		{
			...
			$sizeattr = array(
				...
				'back_totalheight'=>array(),
				
				'back_height'=>array(),
				
				'back_totalwidth'=>array(),
		...
		$this->data['sizeattr'] = array_filter($sizeattr);		
...

	{
		...
		//echo'<pre>'; print_r($product['data']); echo'</pre>'; die();
		...
		if(array_key_exists('published',$product['data'])){
			$product['data']['published'] = 1;
		}else{
			$product['data']['published'] = 0;
		}	
		
		if(array_key_exists('printfulClrCode',$product['data'])){
			$product['data']['printfulClrCode'] = 1;
		}else{
			$product['data']['printfulClrCode'] = 0;
		}	

		if(array_key_exists('printfulprcdepend',$product['data'])){
			$product['data']['printfulprcdepend'] = 1;
		}else{
			$product['data']['printfulprcdepend'] = 0;
		}	

		if(array_key_exists('printfulsize_differ',$product['data'])){
			$product['data']['printfulsize_differ'] = '1';
		}else{
			$product['data']['printfulsize_differ'] = '0';
		}	
		...
		if( isset($product['design']) && (array_key_exists('front',$product['design']) || array_key_exists('back',$product['design']) || array_key_exists('left',$product['design']) || array_key_exists('right',$product['design'])))
		...
		$this->product_m->_table_name = 'sizeattributes';

		$this->product_m->_timestamps = false;


		if (isset($product['sizeattr']) && count($product['sizeattr']) > 0)
		...
		{	
			...
			foreach ($product['sizeattr'] as $sizeattr_key=>$sizeattr_row){

				if($sizeattr_row['size'] != '')
				{

					$sizeattr['size'][] 			= $sizeattr_row['size'];

					$sizeattr['dpi'][] 				= $sizeattr_row['dpi'];

					$sizeattr['width'][] 			= $sizeattr_row['width'];				

					$sizeattr['height'][] 			= $sizeattr_row['height'];						

					$sizeattr['totalwidth'][] 		= $sizeattr_row['totalwidth'];				

					$sizeattr['totalheight'][] 		= $sizeattr_row['totalheight'];	
					
					$sizeattr['back_width'][$sizeattr_key] 	= $sizeattr_row['back_width'];				

					$sizeattr['back_height'][$sizeattr_key] = $sizeattr_row['back_height'];						

					$sizeattr['back_totalwidth'][$sizeattr_key] = $sizeattr_row['back_totalwidth'];				

					$sizeattr['back_totalheight'][$sizeattr_key] = $sizeattr_row['back_totalheight'];						
					
					$sizeattr['price'][] 			= $sizeattr_row['price'];	
				}
...

			{
				...
				$data = array(
					...
					'size' 			=> json_encode($sizeattr['size']),

					'dpi' 			=> json_encode($sizeattr['dpi']),

					'width' 		=> json_encode($sizeattr['width']),

					'totalwidth' 	=> json_encode($sizeattr['totalwidth']),
					
					'totalheight' 	=> json_encode($sizeattr['totalheight']),

					'height' 		=> json_encode($sizeattr['height']),					
					
					'back_width'	=> json_encode($sizeattr['back_width']),
					
					'back_height'   => json_encode($sizeattr['back_height']),
					
					'back_totalwidth' => json_encode($sizeattr['back_totalwidth']),
					
					'back_totalheight'	=> json_encode($sizeattr['back_totalheight']),
					
					'price' 		=> json_encode($sizeattr['price']),
					
					'product_id'	=> $id					
		
				);

				if (isset($sizeattribute->id) && count($sizeattribute))

					$this->product_m->save($data, $sizeattribute->id);
...

		{
			...
			if (count($sizeattribute))

				$this->product_m->delete($sizeattribute->id);
