...
class Frontend_Controller extends MY_Controller
{
	...
	private function cart_common(){
		$cart_cache_id =  $this->session->userdata('cart_cache_id'); 		
		if($cart_cache_id !=''){
			...
			$session_id = explode('_',$cart_cache_id);			
			$check_order = $this->order_m->getOrderByCustom(array('cart_cache_id'=>$cart_cache_id));
			if($check_order->status == 'completed' && $this->cache->get('orders_designs'.$session_id[2])){
				$this->session->unset_userdata(array('cart','order_session_id','cart_cache_id'));
				$this->cart->destroy();
				$this->cache->delete('orders_designs'.$session_id[2]);
			}
		}
	}	
	...
	{		
		...
		{
			$data['meta_robots']	= $settings->meta_robots;
		}
		...
		$this->output->css(base_url('assets/css/custom.css'));
		...
		$this->js('assets/js/html2canvas.js');  
		$this->js('assets/js/Chart.js');  
		$this->js('assets/js/doughnutit.js');		
		$this->js('assets/js/users.js');
		$this->assets();
