							...
							<div class="col-sm-7 view_vector">
								<?php foreach($vector as $value){?>
									<div class="row">
										...
										<?php }elseif(isset($value['thumb'])){ ?>											<?php												$img = $value['thumb'];												if($value['clipartTpe'] == 'uploadedimage'){													$order = $this->db->get_where('dg_order_uploadimage',array('clipart_id'=>$value['clipart_id'],'order_id'=>$product->order_id))->row();													$img = base_url().$order->thumb;																									}?>												<img class="clipart_thumb" src="<?php echo $img; ?>" alt=""/>
										<?php } ?>
											<ul>
												<?php foreach($value as $k=>$v){ 														if($k !=''){																								?>
													<?php if($k != 'text' && $k!='clipartTpe' && $k!='clipart_id' && $k != 'zIndex' && $k != 'type' && $k != 'svg' && $k != 'outlineC' && $k != 'outlineW' && $k != 'title' && $k != 'file_name' && $k != 'file' && $k != 'thumb' && $k != 'url' && $k != 'change_color'){ ?>
